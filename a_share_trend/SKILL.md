...
Tushare API 有频率限制（免费用户通常每分钟 500 次）。本策略已采取以下措施：

1. **自动频率控制**：内置 API 调用间隔控制（默认 0.15 秒/次）
2. **错误重试**：遇到频率限制时，自动等待 10 秒后重试（最多 3 次）
3. **数据缓存**：基础数据（股票列表、行业分类）缓存复用
4. **批量处理**：避免频繁单只查询，尽量批量获取
